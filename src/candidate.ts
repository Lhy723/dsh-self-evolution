...
function validateOperation(value: unknown, index: number): CandidateOperation {
...
export function validateCandidateProposal(
  proposal: CandidateProposal,
  profile: LoadedProfile,
  config: Pick<ResolvedEvolutionConfig,
    'managedFiles' | 'excludedFiles' | 'requiredFiles' | 'maxCandidateOperations' | 'maxCandidateBytes' | 'allowModelRouteMutation'>,
): ValidatedCandidate {
  for (const field of ['hypothesis', 'expectedBehavior', 'summary'] as const) {
  ...
  for (const operation of operations) {
    if (seen.has(operation.path)) throw new EvolutionError(`candidate repeats path ${operation.path}`, 'INVALID_CANDIDATE')
    ...
    if (operation.op === 'upsert') totalBytes += Buffer.byteLength(operation.content ?? '', 'utf8')
  }
  ...
  if (runtimeOperation?.op === 'delete') throw new EvolutionError('runtime.json cannot be deleted', 'INVALID_CANDIDATE')
  if (runtimeOperation?.op === 'upsert') {
    let proposedRuntime: ReturnType<typeof validateRuntime>
    try {
    ...
    if (!config.allowModelRouteMutation) {
      ...
      if (before?.provider !== after?.provider || before?.model !== after?.model) {
        throw new EvolutionError(
          'candidate cannot change runtime agentOptions.provider/model unless allowModelRouteMutation is enabled',
          'INVALID_CANDIDATE',
        )
      }
    }
...
async function rollbackPartial(profileDirectory: string, originals: OriginalFile[]): Promise<void> {
  ...
  for (const original of [...originals].reverse()) {
    ...
    if (original.writtenContent !== undefined) {
      ...
      if (!exists) {
        if (original.existed && original.content !== undefined) await writeFileAtomic(absolute, original.content)
        continue
      }
  ...
  if (conflicts.length > 0) {
    throw new EvolutionError(
      `partial candidate rollback detected concurrent changes in: ${conflicts.join(', ')}`,
      'PROFILE_CHANGED',
      { conflicts },
    )
  }
...
export async function applyCandidate(
  ...
  if (actual !== profile.digest) throw new ConcurrentProfileChangeError(profile.digest, actual)
 ...
 */
export function assertCandidateNotBenchmarkSpecific(
  candidate: ValidatedCandidate,
  benchmark: { digest: string; cases: Array<{ id: string; statement: string; rubric: string }> },
): void {
  const written = candidate.operations
    .filter((operation): operation is CandidateOperation & { op: 'upsert'; content: string } =>
      operation.op === 'upsert' && typeof operation.content === 'string')
  ...
  for (const benchmarkCase of benchmark.cases) {
    ...
    if (statement.length >= 40 && written.includes(statement)) {
      throw new EvolutionError(
        `candidate copies the full public statement for case ${benchmarkCase.id}`,
        'BENCHMARK_OVERFIT',
      )
    }
    if (rubric.length >= 20 && written.includes(rubric)) {
      throw new EvolutionError(
        `candidate contains private rubric text for case ${benchmarkCase.id}`,
        'BENCHMARK_CONTAMINATION',
      )
    }
    if (benchmarkCase.id.length >= 8 && written.includes(`case_id: ${benchmarkCase.id}`)) {
      throw new EvolutionError(
        `candidate contains a benchmark case identifier (${benchmarkCase.id})`,
        'BENCHMARK_OVERFIT',
      )
    }
  }
