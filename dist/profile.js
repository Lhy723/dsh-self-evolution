...
import { canonicalJson, EvolutionError, listRegularFiles, matchesAny, normalizeRelative, pathExists, resolveExistingDirectory, sha256, writeJsonAtomic, } from './util.js';
function assertAgentOptions(value, source) {
    if (value === undefined)
        return;
    ...
    if (candidate.maxTokens !== undefined
        && (!Number.isSafeInteger(candidate.maxTokens) || candidate.maxTokens < 1)) {
        throw new EvolutionError(`${source}.agentOptions.maxTokens must be a positive integer`, 'INVALID_PROFILE');
    }
}
function assertToolFilter(value, source) {
    if (value === undefined)
        return;
    if (value === null || typeof value !== 'object' || Array.isArray(value)) {
...
export function validateRuntime(value, source = 'runtime.json') {
    if (value === null || typeof value !== 'object' || Array.isArray(value)) {
        throw new EvolutionError(`${source} must contain an object`, 'INVALID_PROFILE');
    }
    const candidate = value;
    if (candidate.schemaVersion !== 1)
        throw new EvolutionError(`${source}.schemaVersion must be 1`, 'INVALID_PROFILE');
    ...
    return {
        ...
        ...(candidate.agentOptions === undefined ? {} : { agentOptions: candidate.agentOptions }),
        ...(candidate.toolFilter === undefined ? {} : { toolFilter: candidate.toolFilter }),
        ...(candidate.metadata === undefined ? {} : { metadata: candidate.metadata }),
    };
}
export async function enumerateManagedFiles(profileDirectory, config) {
...
export async function profileFileInventory(profileDirectory, config) {
    const directory = await resolveExistingDirectory(profileDirectory);
...
export async function computeProfileDigest(profileDirectory, config) {
    return digestInventory(await profileFileInventory(profileDirectory, config));
}
export async function loadProfile(profileDirectory, config) {
    const directory = await resolveExistingDirectory(profileDirectory);
...
export function mergeAgentOptions(...options) {
    const result = {};
    let present = false;
...
export function composeTargetPersona(profile) {
...
export function profileAsPublicText(profile, maxBytes = 128 * 1024) {
    const sections = [
        `## AGENTS.md\n${profile.agentsMarkdown.trim()}`,
...
export async function setRuntimeVersion(profileDirectory, version) {
    if (!Number.isSafeInteger(version) || version < 1)
        throw new TypeError('version must be a positive integer');
    const file = path.join(profileDirectory, 'runtime.json');
    if (!await pathExists(file))
        throw new EvolutionError('runtime.json is missing', 'MISSING_PROFILE_FILE');
