...
import { canonicalJson, ConcurrentProfileChangeError, EvolutionError, isPathWithin, nowIso, pathExists, readJson, resolveExistingDirectory, sha256, writeJsonAtomic, } from './util.js';
import { computeProfileDigest, enumerateManagedFiles, profileFileInventory } from './profile.js';
export function snapshotDirectoryForVersion(snapshotsRoot, version) {
...
export async function captureSnapshot(profileDirectory, snapshotsRoot, version, config) {
    ...
    if (await pathExists(target)) {
        ...
        if (existing.profileDigest !== profileDigest) {
            throw new EvolutionError(`snapshot v${version} already exists for a different profile digest`, 'SNAPSHOT_VERSION_CONFLICT', { expected: existing.profileDigest, actual: profileDigest });
        }
...
export async function restoreSnapshot(snapshotDirectory, profileDirectory, expectedCurrentDigest, config) {
    const profile = await resolveExistingDirectory(profileDirectory);
    const actual = await computeProfileDigest(profile, config);
    if (actual !== expectedCurrentDigest)
        throw new ConcurrentProfileChangeError(expectedCurrentDigest, actual);
...
export async function listSnapshotVersions(snapshotsRoot) {
