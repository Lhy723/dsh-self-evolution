...
function requireCallingAgent(agent) {
    if (agent === undefined)
        throw new EvolutionError('self-evolution tools require a calling Agent', 'MISSING_AGENT');
    return agent;
}
function positiveInteger(value, fallback, label) {
    const resolved = value ?? fallback;
    if (!Number.isSafeInteger(resolved) || resolved < 1)
        throw new EvolutionError(`${label} must be a positive integer`, 'INVALID_ARGUMENT');
    return resolved;
}
...
export class SelfEvolutionService extends Service {
    ...
    registerTools(ctx) {
        ...
        ctx.tools.register(defineTool({
            name: runName,
            description: 'Run the complete benchmark-driven self-evolution loop for a managed Agent Profile: establish or reuse a baseline, '
            ...
            async execute(args, exec) {
                ...
                return service.evolve(parent, exec.signal, {
                    ...
                    ...(score(args.target_score) === undefined ? {} : { targetScore: score(args.target_score) }),
        ...
        ctx.tools.register(defineTool({
            name: evaluateName,
            description: 'Evaluate a managed Agent Profile on a frozen benchmark without proposing or applying changes. Runs target and '
        ...
        ctx.tools.register(defineTool({
            name: rollbackName,
            description: 'Restore a managed Agent Profile to one previously verified snapshot version. The operation refuses to overwrite '
            ...
            async execute(args, exec) {
                const parent = requireCallingAgent(exec.agent);
                return service.rollback(parent, exec.signal, service.resolveInputPath(parent, args.profile_dir), positiveInteger(args.version, 0, 'version'));
            },
            presentCall: (args) => ({
                card: 'generic',
