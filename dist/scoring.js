...
export function assertScore(value, label = 'score') {
...
export function aggregateEvaluation(input) {
    const cases = input.benchmark.cases.map(benchmarkCase => {
        ...
        if (runs.length !== input.runsPerCase) {
            throw new EvolutionError(`case ${benchmarkCase.id} has ${runs.length} runs; expected ${input.runsPerCase}`, 'INCOMPLETE_EVALUATION');
        }
...
export function toPublicEvidence(benchmark, reference, rounds) {
    return {
        reference: {
...
export function collectSessionIds(evaluation) {
