...
- The delivery environment had no model endpoint or credentials, so a live LLM/Subagent end-to-end run was not executed. The package must be smoke-tested in a destination profile with its actual provider, sandbox, persistence, and approval policy.

## Recommended destination smoke test
